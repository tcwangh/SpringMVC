package idv.tim.repository.controller;
...
import idv.tim.repository.persistence.ContactRepository;
...
public class HomeController {
	...
	public String home(Locale locale,  @RequestParam(value = "search", required = false) String contactId, Model model) {
		...
		List<Contact> listContact = contactRepository.list();
		logger.info("Contact list size is " + listContact.size() );
		
		if (contactId!=null) {
			List<Contact> theContact = contactRepository.get(Integer.parseInt(contactId));
	...
	public String addContact(Locale locale,@RequestParam("name") String name,
		...
		contactRepository.persist(theContact);
		return "redirect:/";
	}
